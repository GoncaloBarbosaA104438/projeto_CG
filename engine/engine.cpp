...
struct WindowSettings { int width=800, height=800; } windowSettings;
...
struct Polar { double radius, alpha, beta; };
                ...
                const Material& mat)
{
    ...
    if (!file.good()) {
        ...
        if (!file.good()) { printf("Cannot open model: %s\n", filename); exit(1); }
    }
    ...
    verts.reserve(N*3); norms.reserve(N*3); texcs.reserve(N*2);
    ...
    for (unsigned long i = 0; i < N; i++) {
        ...
        texcs.push_back(s); texcs.push_back(t);
    }
    file.close();
...
static Material parseMaterial(XMLElement* modelEl)
{
    ...
    if (sh) mat.shininess = sh->FloatAttribute("value", 0.0f);
    return mat;
}
    ...
    if (tr) {
        ...
        while (op) {
            ...
            if (name == "translate") {
                ...
                t.y = op->FloatAttribute("y");
                t.z = op->FloatAttribute("z");
                if (op->Attribute("time")) t.time  = op->FloatAttribute("time");
                else                       t.angle = op->FloatAttribute("angle");
            } else if (name == "scale") {
                t.type = SCALE;
                t.x = op->FloatAttribute("x");
                t.y = op->FloatAttribute("y");
    ...
    if (modelsEl) {
        ...
        while (modelEl) {
            ...
            if (file) {
                ...
                if (texEl) texFile = texEl->Attribute("file");
...
bool loadScene(const char* filename)
{
    ...
    if (cam) {
        auto qf=[](XMLElement*e,const char*a,float*v){if(e)e->QueryFloatAttribute(a,v);};
        ...
        qf(pos,"x",&cameraSettings.posX); qf(pos,"y",&cameraSettings.posY); qf(pos,"z",&cameraSettings.posZ);
        qf(look,"x",&cameraSettings.lookX); qf(look,"y",&cameraSettings.lookY); qf(look,"z",&cameraSettings.lookZ);
        qf(up,"x",&cameraSettings.upX);  qf(up,"y",&cameraSettings.upY);  qf(up,"z",&cameraSettings.upZ);
        if (proj) {
        ...
        camPos.radius=r;
        if (r>0){camPos.beta=asin(cameraSettings.posY/r);camPos.alpha=atan2(cameraSettings.posX,cameraSettings.posZ);}
    }
    ...
    if (rootGroup) sceneRoot = parseGroup(rootGroup);
...
void drawGroup(const Group& g, float t)
{
        ...
        if (tr.type == TRANSLATE) {
            if (tr.time > 0 && tr.curvePoints.size() >= 4) {
                ...
                if (tr.align) {
                    ...
                    normalize3(x);
                    float up[3]={0,1,0};
                    cross3(x, up, z); normalize3(z);
                    cross3(z, x,  y); normalize3(y);
    ...
    for (const auto& m : g.models) {
        ...
        if (m.textureID) glDisable(GL_TEXTURE_2D);
    }
...
{
    if (key == '+' && camPos.radius > 1) camPos.radius -= 1;
...
{
    if (argc != 2) { printf("Uso: ./engine <scene.xml>\n"); return 1; }
    ...
    if (!loadScene(argv[1])) { puts("Erro ao carregar XML!"); return 1; }
    ...
    float globalAmb[4] = {0.1f, 0.1f, 0.1f, 1.0f};
